      ...
      next: (res: any) => {
        if (res && res.id) {
          if (this.wishId) {  // 有許願，先結案再跳轉
            const finishUrl = 'http://localhost:8080/gogobuy/wish/finish_wish?id=${this.wishId}&userId=${this.userId}&targetUrl=localhost:4200/groupbuy-event/group-follow/${res.id}';
